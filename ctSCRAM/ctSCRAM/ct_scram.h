 ...
 */
...
#if defined(CT_SCRAM_DLL) && defined(_CT_SCRAM_BUILD_DLL)
  ...
  */
 #error "You may not have both CT_SCRAM_DLL and _CT_SCRAM_BUILD_DLL defined"
#endif

/* CT_SCRAM_API is used to declare public API functions for export
 * from the DLL / shared library / dynamic library.
 */
#if defined(_WIN32) && defined(_CT_SCRAM_BUILD_DLL)
 /* We are building crMath as a Win32 DLL */
 #define CT_SCRAM_API __declspec(dllexport)
#elif defined(_WIN32) && defined(CT_SCRAM_DLL)
 /* We are calling crMath as a Win32 DLL */
 #define CT_SCRAM_API __declspec(dllimport)
#elif defined(__GNUC__) && defined(_CT_SCRAM_BUILD_DLL)
 /* We are building CT_SCRAM as a shared / dynamic library */
 #define CT_SCRAM_API __attribute__((visibility("default")))
#else
 /* We are building or calling crMath as a static library */
 #define CT_SCRAM_API extern
#endif

//inline doesn't exist in C89, __inline is MSVC specific
#ifndef CT_SCRAM_INLINE
#ifdef _WIN32
#define CT_SCRAM_INLINE __inline
#else
#define CT_SCRAM_INLINE
#endif
#endif

//inline doesn't exist in C89, __inline is MSVC specific
#ifndef CT_SCRAM_DECLSPEC
#ifdef _WIN32
#define CT_SCRAM_DECLSPEC __declspec
#else
#define CT_SCRAM_DECLSPEC
#endif
#endif

//align functions are diffent on windows vs iOS, Linux, etc.
#ifndef CT_SCRAM_ALIGN//(X)
#ifdef _WIN32
#define CT_SCRAM_ALIGN(X) (align(X))
#else
#define CT_SCRAM_ALIGN(X) __attribute__ ((aligned(X)))
#endif
#endif
