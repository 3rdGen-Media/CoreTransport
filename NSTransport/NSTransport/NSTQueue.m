...
void* __stdcall NST_Dequeue_Resolve_Connect_Handshake(LPVOID lpParameter)
{
    //int i;
    OSStatus status;
    ...
    while(1)
    {
        ...
        fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::Waiting for cursor event\n");
    
        //wait for active target and read events with zero timeout
        if( (ret = kqueue_wait_with_timeout(cxQueue, &(kev[0]), 2, 0)) < 0 )
        {
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout 1 failed with ret (%d) and errno (%d).\n", ret, errno);
            assert(1==0);
        }
        else if(ret == 1)
        {
        ...
        if( kev[0].filter == EVFILT_USER )
        {
            ...
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::Read overlappedTarget cursor (%llu) from cxQueue\n", cursor->queryToken);
            ...
            if( overlappedTarget->stage != CT_OVERLAPPED_RECV_FROM && overlappedTarget->stage != CT_OVERLAPPED_EXECUTE) //skip over this for async recv_from completion
            {
                ...
                fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::Waiting for read event\n");
                assert(1==0);
                ...
                {
                    fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout cxQueue read failed with ret (%d).\n", ret);
                    assert(1==0);
                }
                //fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout 2 ret = (%hd) errno = %d.\n", kev[1].filter, errno);

                fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout cxQueue read filter = (%hd) errno = %ld.\n", kev[1].filter, kev[1].data);
                ...
                fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::Bytes Available\n");
                ...
                fprintf(stderr, "**** nNST_Dequeue_Resolve_Connect_Handshake::NumBytesTransferred C status = %lu\n", NumBytesTransferred);
                fprintf(stderr, "**** nNST_Dequeue_Resolve_Connect_Handshake::ictlsocket C status = %d\n", ioRet);
                fprintf(stderr, "**** nNST_Dequeue_Resolve_Connect_Handshake::ictlsocket C = %lu\n", ioctlBytes);
        ...
        {
            fprintf(stderr, "\nnNST_Dequeue_Resolve_Connect_Handshake::Waiting for overlapped pipe\n");
            ...
            {
                fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout 0 failed with ret (%d).\n", ret);
                assert(1==0);
            }
            ...
            if( bytes == -1 )
            {
                fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::Read from overlapped pipe failed with error %d\n", errno);
                assert(1==0);
            }
            ...
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::Read connection target (%s) from cxPipe\n", overlappedTarget->target->url.host);
        }
    

        if (overlappedTarget->stage == CT_OVERLAPPED_SCHEDULE)
        {
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::CT_OVERLAPPED_SCHEDULE\n");
        ...
        {
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::CT_OVERLAPPED_RECV_FROM\n");
            //assert(1 == 0);
            cursor = overlappedTarget->cursor;
            cursor->target = overlappedTarget->target;
            /*
            ...
            */
            ...
            {
                fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::dill_ipaddr_dns_query_wait_ai failed to resolve any IPV4 addresses!\n");
            ...
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::DNS resolved %d ip address(es)\n", numResolvedAddresses);
        ...
        {
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::CT_OVERLAPPED_EXECUTE\n");
            ...
            {
                fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout cxQueue write failed with ret (%d).\n", ret);
                assert(1==0);
            }

            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout cxQueue write filter = (%hd) errno = %ld.\n", kev[1].filter, kev[1].data);
            ...
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::ConnectEx complete error = %d\n\n", CTSocketError());
            ...
            if ((err.id = CTSocketGetError(overlappedTarget->target->socket)) != 0)
            {
                ...
                fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::socket async connect failed with result: %d", err.id);
                CTSocketClose(overlappedTarget->target->socket);
                err.id = CTSocketConnectError;
                goto CONN_CALLBACK;
            }

            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::After CTSocketGetError\n\n");
            ...
            */
            ...
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::After CreateIoCompletionPort\n\n");
...
#ifdef _DEBUG
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::Before CTSSLRoutine Host = %s\n", conn->socketContext.host);
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::Before CTSSLRoutine Host = %s\n", overlappedTarget->target->url.host);
#endif
            ...
            if (conn->target->proxy.host)
            {
                ...
                {
                    fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::CTSSLRoutine failed with error: %d\n", (int)status);
        ...
        {
            fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::UNHANDLED_STAGE (%d)\n", overlappedTarget->stage);
            fflush(stderr);
        }
        /*
        if (NumBytesSent == 0)
    ...
    fprintf(stderr, "\nNST_Dequeue_Resolve_Connect_Handshake::End\n");
...
void* __stdcall NST_Dequeue_Recv_Decrypt(LPVOID lpParameter)
{
    //uintptr_t event;
    //int i;
    ...
    NSTConnection* nstConn = NULL;// (CXConnection*)lpParameter;
    ...
    fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Begin\n");
    ...
    while(1)
    {
        ...
        fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Waiting for cursor event\n");
        ...
        */
        ...
        nstConn = (NSTConnection*)(cursor->conn->object_wrapper);
        ...
        fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Read cursor (%lu) from rxQueue\n", cursor->queryToken);
#else
        //wait for active cursor and read events with zero timeout
        if( (ret = kqueue_wait_with_timeout(rxQueue, &(kev[0]), 2, 0)) < 0 )
        {
            fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::kqueue_wait_with_timeout 1 failed with ret (%d) and errno (%d).\n", ret, errno);
            assert(1==0);
        }
        else if(ret == 1)
        {
        ...
        if( kev[0].filter == EVFILT_USER ) {
            ...
            //Get the cursor/connection that has been pinned on the overlapped
            cursor = (CTCursor*)overlappedResponse->cursor;
            nstConn = (__bridge NSTConnection*)(cursor->conn->object_wrapper);

            assert(overlappedResponse);
            assert(cursor);

            fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Read cursor (%llu) from rxQueue\n", cursor->queryToken);
        }
        else
        {
            fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Waiting for overlapped pipe\n");
            ...
            {
                fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::kqueue_wait_with_timeout 0 failed with ret (%d).\n", ret);
                assert(1==0);
            }
            ...
            if( bytes == -1 )
            {
                fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Read from overlapped pipe failed with error %d\n", errno);
                assert(1==0);
            }
            //fprintf(stderr, "\nCT_Dequeue_Recv_Decrypt::Read %d bytes from overlapped pipe \n", (int)bytes);
            ...
            fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Read cursor (%llu) from rxPipe\n", cursor->queryToken);
        }

        fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Waiting for read event\n");

        //Wait for kqueue notification that bytes are available for reading on rxQueue
        if( kev[1].filter != EVFILT_READ && (ret = kqueue_wait_with_timeout(rxQueue, &(kev[1]), 1, CTSOCKET_MAX_TIMEOUT)) != 1 )
        {
            fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::kqueue_wait_with_timeout rxQueue read failed with ret (%d).\n", ret);
            assert(1==0);
        }
        //fprintf(stderr, "\nCT_Dequeue_Recv_Decrypt::kqueue_wait_with_timeout 2 ret = (%hd) errno = %d.\n", kev[1].filter, errno);

        fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::kqueue_wait_with_timeout rxQueue read filter = (%hd) errno = %ld.\n", kev[1].filter, kev[1].data);
        ...
        if( overlappedResponse ) {
            ...
            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::cursor = (%d)\n", (int)cursor->queryToken);
            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::overlappedResponse->stage = (%d)\n", (int)overlappedResponse->stage);

            if (overlappedResponse->stage > CT_OVERLAPPED_STAGE_NULL && overlappedResponse->stage < CT_OVERLAPPED_SCHEDULE_RECV_FROM)
            {
                fprintf(stderr, "NST_Dequeue_Recv_Decrypt::Scheduling Cursor (%d) via ...", (int)cursor->queryToken);
                ...
                else
                {
                    ...
                    fprintf(stderr, "**** NST_Dequeue_Recv_Decrypt::NumBytesRecv C status = %lu\n", NumBytesRecv);
                    fprintf(stderr, "**** NST_Dequeue_Recv_Decrypt::ictlsocket C status = %d\n", ioRet);
                    fprintf(stderr, "**** NST_Dequeue_Recv_Decrypt::ictlsocket C = %lu\n", ioctlBytes);
                    ...
                    if ((ctError = CTCursorAsyncRecv(&overlappedResponse, (void*)(cursor->file.buffer), 0, &NumBytesRecv)) != CTSocketIOPending)
                    {
                        ...
                        {
                            //cursor->conn->rxCursorQueueCount++;
                            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CTAsyncRecv returned immediate with %lu bytes\n", NumBytesRecv);
                        }
                        else //failure
                        {
                            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CTAsyncRecv failed with CTDriverError = %d\n", ctError);
                        }
                    }
#else
                    cursor->conn->responseCount++;
                ...
                while (NumBytesRemaining > 0 && scRet == SEC_E_OK)
                {
                    ...
                    if (cursor->headerLength == 0)
                    {
                        ...
                        if (nstConn)
                        {
                            ...
                            NSTCursor * nstCursor = ([nstConn getRequestCursorForKey:cursor->queryToken]);
                            endOfHeader = [nstCursor ProcessResponseHeader:cursor->file.buffer Length:NumBytesRecv Cursor:cursor];
                        }
                        else
                        {
                        ...
                        if (endOfHeader)
                        {
                            ...
                            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV Header = \n\n%.*s\n\n", (int)cursor->headerLength, cursor->file.buffer);
                            ...
                            if (endOfHeader != cursor->file.buffer)
                            {
                                ...
                                else
                                {
                                    ...
                                    fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV Body (%lu)\n\n", cursor->contentLength);
                    ...
                    fprintf(stderr, "NST_Dequeue_Recv_Decrypt::Check cursor->contentLength (%d, %d) \n\n", (int)cursor->contentLength, (int)(overlappedResponse->buf - cursor->file.buffer));
                    if (cursor->contentLength > 0 && ((int)cursor->contentLength) <= (int)(overlappedResponse->buf - cursor->file.buffer))
                    {
                        ...
                        fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Waiting for nextCursor event on pipe\n");
...
//#ifdef _DEBUG
                            ...
                            assert(nextCursor);
//#endif
                            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::cursor = (%lu)\n", (unsigned long)cursor->queryToken);
                            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::nextCursor = (%lu)\n", (unsigned long)nextCursor->queryToken);
                            ...
                            {
                                fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV::vagina (%ld)\n", (overlappedResponse->buf - cursor->file.buffer));
                                ...
                                fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV::buffer overlap (%d)\n", (int)cursor->conn->response_overlap_buffer_length);
                                ...
                                if (nextCursor && cursor->conn->response_overlap_buffer_length > 0)
                                {
                                    ...
                                    if (nextCursor->headerLength == 0)
                                    {
                                        ...
                                        if (nstConn)
                                        {
                                            ...
                                            NSTCursor* nstCursor = [nstConn getRequestCursorForKey:nextCursor->queryToken];
                                            endOfHeader = [nstCursor ProcessResponseHeader:nextCursor->file.buffer Length:cursor->conn->response_overlap_buffer_length Cursor:nextCursor];
                                        }
                                        else
                                        ...
                                        if (endOfHeader)
                                        {
                                            ...
                                            if (nextCursor->headerLength > 0)
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV nextCursor->Header = \n\n%.*s\n\n", (int)nextCursor->headerLength, nextCursor->requestBuffer);
                                        ...
                                        fprintf(stderr, "NST_Dequeue_Recv_Decrypt::Advance nextCursor->overlappedResponse.buf\n");
                                    }
                            ...
                            {
                                fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV::Penis\n");
                                ...
                                if ((scRet = CTCursorAsyncRecv(&(overlappedResponse), (void*)(nextCursor->overlappedResponse.buf/* + cursor->conn->response_overlap_buffer_length - nextCursor->headerLength*/), 0, &NumBytesRecv)) != CTSocketIOPending)
                                {
                                    ...
                                    if (scRet == CTSuccess)
                                    {
                                        fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV::CTAsyncRecv (next cursor) returned immediate with %lu bytes\n", NumBytesRecv);
                                    }
                                    else //failure
                                    {
                                        fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV::CTAsyncRecv (next cursor) failed with CTDriverError = %d\n", (int)scRet);
                                        assert(1 == 0);
                                    }
                                }
                            ...
                            {
                                fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV::Scrotum\n");
...
//#endif
...
#endif
                        //issue the response to the client caller
                        if (nstConn) [nstConn distributeResponseWithCursorForToken:closeCursor->queryToken];
                    ...
                    else
                    {
                        ...
                        fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_RECV::wanker\n");
            ...
            else if (overlappedResponse->stage == CT_OVERLAPPED_EXECUTE)//CT_OVERLAPPED_RECV_DECRYPT)
            {
                ...
                */
                ...
                while (NumBytesRemaining > 0 && scRet == SEC_E_OK)
                {
                    ...
                    fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE C::Decrypted data: %lu bytes recvd, %lu bytes remaining, scRet = 0x%x\n", NumBytesRecv, NumBytesRemaining, (int)scRet); //PrintText(NumBytesRecv, (PBYTE)(overlappedResponse->wsaBuf.buf + overlappedResponse->conn->sslContext->Sizes.cbHeader));
                        ...
                        */
                        ...
                        if (cursor->headerLength == 0)
                        {
                            ...
                            if (endOfHeader)
                            {
                                ...
                                fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE Header = \n\n%.*s\n\n", (int)cursor->headerLength, cursor->file.buffer);
                        ...
                        if (cursor->contentLength > 0 && ((int)cursor->contentLength) <= (int)(overlappedResponse->buf - cursor->file.buffer))
                        {
                            ...
                            */
                            ...
                            if( (ret = kqueue_wait_with_timeout(pendingCursorQueue, &nextCursorMsg, 1, 0)) < 0 )
                            {
                                fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::kqueue_wait_with_timeout nextCursorMsg failed with ret (%d).\n", ret);
                                assert(1==0);
                            }
                            //if (PeekMessage(&nextCursorMsg, NULL, WM_USER, WM_USER + cursor->conn->queryCount - 1, PM_REMOVE)) //(nextCursor)
                            if( ret == 1)
                            {
                                ...
                                if( bytes == -1 )
                                {
                                    fprintf(stderr, "\nNST_Dequeue_Recv_Decrypt::Read from overlapped pipe (nextCursorMsg) failed with error %d\n", errno);
                                    assert(1==0);
                                }
                                //fprintf(stderr, "\nCT_Dequeue_Recv_Decrypt::Read %d bytes from overlapped pipe \n", (int)bytes);
                                assert(bytes == sizeof(CTOverlappedResponse));
                                ...
                                if (cursor->contentLength < overlappedResponse->buf - cursor->file.buffer)
                                {
                                    fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::vagina (%ld)\n", (overlappedResponse->buf - cursor->file.buffer));

                                    cursor->conn->response_overlap_buffer_length = (size_t)overlappedResponse->buf - (size_t)(cursor->file.buffer + cursor->contentLength);
                                    assert(cursor->conn->response_overlap_buffer_length > 0);

                                    fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::buffer overlap (%d)\n", (int)cursor->conn->response_overlap_buffer_length);
                                            ...
                                            if (endOfHeader)
                                            {
                                                ...
                                                if (nextCursor->headerLength > 0)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE nextCursor->Header = \n\n%.*s\n\n", (int)nextCursor->headerLength, nextCursor->requestBuffer);
                                                        ...
                                                        if (cursor->conn->response_overlap_buffer_length > nextCursor->headerLength && endOfHeader != nextCursor->file.buffer)
                                                        {
                                                            ...
                                                            //fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_RECV nextCursor->Body (%lu) = \n\n%.*s\n\n", cursor->conn->response_overlap_buffer_length, (int)nextCursor->contentLength, nextCursor->file.buffer);
                                                            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE nextCursor->Body (%lu)\n\n", cursor->conn->response_overlap_buffer_length);
                                ...
                                if (nextCursor && NumBytesRemaining == 0)
                                {
                                    fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::Penis\n");
                                    //assert(1==0);
                                    ...
                                    if ((scRet = CTCursorAsyncRecv(&(overlappedResponse), (void*)(nextCursor->overlappedResponse.buf/* + cursor->conn->response_overlap_buffer_length - nextCursor->headerLength*/), 0, &NumBytesRecv)) != CTSocketIOPending)
                                    {
                                        ...
                                        if (scRet == CTSuccess)
                                        {
                                            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::CTAsyncRecv (next cursor) returned immediate with %lu bytes\n", NumBytesRecv);
                                        }
                                        else //failure
                                        {
                                            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::CTAsyncRecv (next cursor) failed with CTDriverError = %d\n", (int)scRet);
                                            assert(1 == 0);
                                        }
                                    }
...
//#endif
                        ...
                        else if (NumBytesRemaining == 0)
                        {
                            ...
                            fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::wanker\n");
                            fprintf(stderr, "\n\n\n\n\n\n\n\n\n\n READ MORE PATH NumBytesRecv(%lu) NumBytesRemaning (%lu)...\n\n\n\n\n\n\n\n\n\n", NumBytesRecv, NumBytesRemaining);
                            ...
                            if ((scRet = CTCursorAsyncRecv(&overlappedResponse, overlappedResponse->buf, 0, &NumBytesRemaining)) != CTSocketIOPending)
                            {
                                ...
                                if (scRet == CTSuccess)
                                {
                                    fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::CTAsyncRecv returned immediate with %lu bytes\n", NumBytesRemaining);
                                    //scRet = CTSSLDecryptMessage(overlappedResponse->conn->sslContext, overlappedResponse->wsaBuf.buf, &NumBytesRecv);
                                    //continue;
                                }
                                else //failure
                                {
                                    fprintf(stderr, "NST_Dequeue_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::CTAsyncRecv failed with CTDriverError = %d\n", (int)scRet);
                                    assert(1 == 0);
                                }
                            }
                ...
                if (scRet == SEC_E_INCOMPLETE_MESSAGE)
                {
                    ...
                    if ((scRet = CTCursorAsyncRecv(&overlappedResponse, overlappedResponse->buf, NumBytesRecv, &NumBytesRemaining)) != CTSocketIOPending)
                    {
                        ...
                        {
                            fprintf(stderr, "CXConnection::DecryptResponseCallback::CTAsyncRecv failed with CTDriverError = %d\n", (int)scRet);
                            break;
                        }
                    }
                }
...
void* __stdcall NST_Dequeue_Encrypt_Send(LPVOID lpParameter)
{
    uint64_t queryToken;
    //uintptr_t event;
    CTCursor* cursor = NULL;
    ...
    while( 1)
    {
        ...
        fprintf(stderr, "\nNST_Dequeue_Encrypt_Send(kqueue)::Waiting for cursor\n");
        ...
        if( kev.ident != CTSOCKET_EVT_TIMEOUT  )
        {
            ...
            {
                fprintf(stderr, "\nNST_Dequeue_Encrypt_Send(kqueue) continue\n");
                continue;
            }
            ...
            {
                fprintf(stderr, "\nNST_Dequeue_Encrypt_Send(kqueue)::Server Disconnected!!!\n");
                assert(1==0);
            }
            else
            {
...
#endif
                    {
                        fprintf(stderr, "**** NST_Dequeue_Encrypt_Send(kqueue)::CT_OVERLAPPED_SCHEDULE_SEND Error %d sending data to server (%ld)\n", CTSocketError(), cbData);
                        assert(1==0);
                    }

                    fprintf(stderr, "NST_Dequeue_Encrypt_Send::%ld bytes of handshake data sent\n", cbData);
