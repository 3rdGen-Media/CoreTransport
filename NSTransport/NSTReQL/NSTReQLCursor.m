...
@implementation NSTReQLCursor : NSTCursor
...
{
    return [[NSTReQLCursor alloc] initWithConnection:conn];
}

+(NSTReQLCursor*)newCursor:(NSTConnection*)conn filePath:(const char*)filepath
{
    return [[NSTReQLCursor alloc] initWithConnection:conn andFilePath:filepath];
}
...
-(char*)ProcessResponseHeader:(char*)buffer Length:(unsigned long)bufferLength Cursor:(CTCursor*)cursor
{
    ...
    uint64_t cursor_token = self.cursor->queryToken;
    ...
    self.cursor->contentLength = ((ReqlQueryMessageHeader*)buffer)->length;
