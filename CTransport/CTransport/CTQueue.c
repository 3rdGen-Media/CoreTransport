...
void* __stdcall CT_Dequeue_Resolve_Connect_Handshake(LPVOID lpParameter)
{
	//int i;
	OSStatus status;
	...
	{    
		...
		fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::Waiting for cursor event\n");
	
		//wait for active target and read events with zero timeout 
		if( (ret = kqueue_wait_with_timeout(cxQueue, &(kev[0]), 2, 0)) < 0 )
		{
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout 1 failed with ret (%d) and errno (%d).\n", ret, errno);
			assert(1==0);
		}
		else if(ret == 1)
		{
		...
		if( kev[0].filter == EVFILT_USER )
		{
			...
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::Read overlappedTarget cursor (%llu) from cxQueue\n", cursor->queryToken);
			...
			if( overlappedTarget->stage != CT_OVERLAPPED_RECV_FROM && overlappedTarget->stage != CT_OVERLAPPED_EXECUTE) //skip over this for async recv_from completion
			{
				...
				fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::Waiting for read event\n");
				assert(1==0);
				...
				{
					fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout cxQueue read failed with ret (%d).\n", ret);
					assert(1==0);
				}
				...
				fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::Bytes Available\n");
		...
		{
			fprintf(stderr, "\nnCT_Dequeue_Resolve_Connect_Handshake::Waiting for overlapped pipe\n");
			...
			{
				fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout 0 failed with ret (%d).\n", ret);
				assert(1==0);
			}
			...
			if( bytes == -1 )
			{
				fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::Read from overlapped pipe failed with error %d\n", errno);
				assert(1==0);
			}
			...
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::Read connection target (%s) from cxPipe\n", overlappedTarget->target->url.host);
		}
	

		if (overlappedTarget->stage == CT_OVERLAPPED_SCHEDULE)
		{
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::CT_OVERLAPPED_SCHEDULE\n");
		...
		{
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::CT_OVERLAPPED_RECV_FROM\n");
			//assert(1 == 0);
			cursor = overlappedTarget->cursor;
			cursor->target = overlappedTarget->target;
			/*
		...
		{
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::CT_OVERLAPPED_EXECUTE\n");
			...
			{
				fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::kqueue_wait_with_timeout cxQueue write failed with ret (%d).\n", ret);
				assert(1==0);
			}
			...
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::ConnectEx complete error = %d\n\n", CTSocketError());
			...
			if ((error.id = CTSocketGetError(overlappedTarget->target->socket)) != 0)
			{
				...
				fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::socket async connect failed with result: %d", error.id);
				CTSocketClose(overlappedTarget->target->socket);
				error.id = CTSocketConnectError;
				goto CONN_CALLBACK;
			}

			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::After CTSocketGetError\n\n");
			...
			*/
			...
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::After CreateIoCompletionPort\n\n");
...
#ifdef _DEBUG
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::Before CTSSLRoutine Host = %s\n", conn->socketContext.host);
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::Before CTSSLRoutine Host = %s\n", overlappedTarget->target->url.host);
#endif
			...
			if (conn->target->proxy.host)
			{
				...
				{
					fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::CTSSLRoutine failed with error: %d\n", (int)status);
		...
		{
			fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::UNHANDLED_STAGE (%d)\n", overlappedTarget->stage);
			fflush(stderr);
		}
		/*
		if (NumBytesSent == 0)
	...
	fprintf(stderr, "\nCT_Dequeue_Resolve_Connect_Handshake::End\n");
...
void* __stdcall CT_Dequeue_Recv_Decrypt(LPVOID lpParameter)
{
	...
	{    
        ...
        //wait for lio_listio/aio_read completion event with infinite timeout
		...
		*/
		...
		fprintf(stderr, "\nCT_Dequeue_Recv_Decrypt::Read cursor (%llu) from rxQueue\n", cursor->queryToken);
#else
		...
		else
		{
			...
			fprintf(stderr, "\nCT_Dequeue_Recv_Decrypt::Read cursor (%llu) from rxPipe\n", cursor->queryToken);
		}
...
//#endif					
							...
							if (nextCursor && NumBytesRemaining == 0)
							{
								...
								if ((scRet = CTCursorAsyncRecv(&(overlappedResponse), (void*)(nextCursor->overlappedResponse.buf/* + cursor->conn->response_overlap_buffer_length - nextCursor->headerLength*/), 0, &NumBytesRecv)) != CTSocketIOPending)
								{
									...
									if (scRet == CTSuccess)
									{
										fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_RECV::CTAsyncRecv (next cursor) returned immediate with %lu bytes\n", NumBytesRecv);
									}
									else //failure
									{
										fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_RECV::CTAsyncRecv (next cursor) failed with CTDriverError = %ld\n", scRet);
										assert(1 == 0);
									}
								}
...
//#endif
					...
					else
					{
						...
						if ((scRet = CTCursorAsyncRecv(&overlappedResponse, overlappedResponse->buf, 0, &NumBytesRemaining)) != CTSocketIOPending)
						{
							...
							if (scRet == CTSuccess)
							{
	 							fprintf(stderr, "CT_Deque_Recv_Decrypt::CTAsyncRecv returned immediate with %lu bytes\n", NumBytesRemaining);
								//scRet = CTSSLDecryptMessage(overlappedResponse->conn->sslContext, overlappedResponse->wsaBuf.buf, &NumBytesRecv);
								//continue;
							}
							else //failure
							{
								fprintf(stderr, "CT_Deque_Recv_Decrypt::CTAsyncRecv failed with CTDriverError = %ld\n", scRet);
								assert(1 == 0);
							}
						}
						...
						*/
						...
						if (cursor->headerLength == 0)
						{
							...
							if (endOfHeader)
							{
								...
								fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE Header = \n\n%.*s\n\n", (int)cursor->headerLength, cursor->file.buffer);
							...
							*/
							...
							if( ret == 1)
							{
                                ...
                                if (cursor->contentLength < overlappedResponse->buf - cursor->file.buffer)
								{
									...
									fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::buffer overlap (%d)\n", (int)cursor->conn->response_overlap_buffer_length);
											...
											if (endOfHeader)
											{
												...
												if (nextCursor->headerLength > 0)
												{
													...
													else
													{
														...
														fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE nextCursor->Header = \n\n%.*s\n\n", (int)nextCursor->headerLength, nextCursor->requestBuffer);
														...
														if (cursor->conn->response_overlap_buffer_length > nextCursor->headerLength && endOfHeader != nextCursor->file.buffer)
														{
															...
															//fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_RECV nextCursor->Body (%lu) = \n\n%.*s\n\n", cursor->conn->response_overlap_buffer_length, (int)nextCursor->contentLength, nextCursor->file.buffer);
															fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE nextCursor->Body (%lu)\n\n", cursor->conn->response_overlap_buffer_length);
								...
								if (nextCursor && NumBytesRemaining == 0)
								{
									fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::Penis\n");
									//assert(1==0);
									...
									if ((scRet = CTCursorAsyncRecv(&(overlappedResponse), (void*)(nextCursor->overlappedResponse.buf/* + cursor->conn->response_overlap_buffer_length - nextCursor->headerLength*/), 0, &NumBytesRecv)) != CTSocketIOPending)
									{
										...
										if (scRet == CTSuccess)
										{
											fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::CTAsyncRecv (next cursor) returned immediate with %lu bytes\n", NumBytesRecv);
										}
										else //failure
										{
											fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::CTAsyncRecv (next cursor) failed with CTDriverError = %ld\n", scRet);
											assert(1 == 0);
										}
									}
...
//#endif
						...
						else if (NumBytesRemaining == 0)
						{
							...
							fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::wanker\n");
							fprintf(stderr, "\n\n\n\n\n\n\n\n\n\n READ MORE PATH NumBytesRecv(%lu) NumBytesRemaning (%lu)...\n\n\n\n\n\n\n\n\n\n", NumBytesRecv, NumBytesRemaining);
							...
							if ((scRet = CTCursorAsyncRecv(&overlappedResponse, overlappedResponse->buf, 0, &NumBytesRemaining)) != CTSocketIOPending)
							{
								...
								if (scRet == CTSuccess)
								{
									fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::CTAsyncRecv returned immediate with %lu bytes\n", NumBytesRemaining);
									//scRet = CTSSLDecryptMessage(overlappedResponse->conn->sslContext, overlappedResponse->wsaBuf.buf, &NumBytesRecv);
									//continue;
								}
								else //failure
								{
									fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_EXECUTE::CTAsyncRecv failed with CTDriverError = %ld\n", scRet);
									assert(1 == 0);
								}
							}
...
#endif
					{
						fprintf(stderr, "**** CT_Dequeue_Encrypt_Send(kqueue)::CT_OVERLAPPED_SCHEDULE_SEND Error %d sending data to server (%ld)\n", CTSocketError(), cbData);
						assert(1==0);
					}

                    fprintf(stderr, "CT_Dequeue_Encrypt_Send::%ld bytes of handshake data sent\n", cbData);
...
volatile int64_t inflightResponseCount = 0;
...
unsigned long __stdcall CT_Dequeue_Recv_Decrypt(LPVOID lpParameter)
{
	...
	//TO DO:  Optimize number of packets dequeued at once
	while (GetQueuedCompletionStatusEx(rxQueue, ovl_entry, CT_MAX_INFLIGHT_DECRYPT_PACKETS, &nPaquetsDequeued, INFINITE, TRUE))
	{
		...
		for (i = 0; i < nPaquetsDequeued; i++)
		{
			...
			//cursor->conn->socketContext.rq = *rq;
			//cursor->conn->target->rq = *rq;
			...
			{
				fprintf(stderr, "CTConnection::CTDecryptResponseCallback::Scheduling Cursor (%d, %llu) via ...", cursor->conn->socket, cursor->queryToken);
				...
				while (NumBytesRemaining > 0 && scRet == SEC_E_OK)
				{
					...
					if (cursor->headerLength == 0)
					{
						...
						if (endOfHeader)
						{
							...
							fprintf(stderr, "CT_Deque_Recv_Decrypt::CT_OVERLAPPED_RECV Header (%d) = \n\n%.*s\n\n", (int)cursor->headerLength, (int)cursor->headerLength, cursor->file.buffer);
							...
							if (cursor->contentLength < overlappedResponse->buf - cursor->file.buffer)
							{
								...
								if (nextCursorOnConn && cursor->conn->response_overlap_buffer_length > 0)
								{
									...
									if (nextCursorOnConn->headerLength == 0)
									{
										...
										char* endOfHeader = nextCursorOnConn->headerLengthCallback(nextCursorOnConn, nextCursorOnConn->file.buffer, cursor->conn->response_overlap_buffer_length);
										...
										if (endOfHeader)
										{
											...
											if (nextCursorOnConn->headerLength > 0)
											{
												...
												else
												{
													...
													//this print statement causes windows system chime on 8th ReQL call
												...
												nextCursorOnConn->overlappedResponse.buf = nextCursorOnConn->file.buffer + nextCursorOnConn->conn->response_overlap_buffer_length;
						...
						if (cursor->contentLength <= overlappedResponse->buf - cursor->file.buffer)
						{
							...
							if (PeekMessage(&nextCursorMsg, (HWND)-1, 0, 0, PM_REMOVE))
							{
								...
								if (cursor->contentLength < overlappedResponse->buf - cursor->file.buffer)
								{
									...
									if (nextCursorOnConn && cursor->conn->response_overlap_buffer_length > 0)
									{
										assert(nextCursorOnConn->conn == cursor->conn);
										//copy overlapping cursor's response from previous cursor's buffer to next cursor's buffer
										memcpy(nextCursorOnConn->file.buffer, cursor->file.buffer + cursor->contentLength, cursor->conn->response_overlap_buffer_length);
