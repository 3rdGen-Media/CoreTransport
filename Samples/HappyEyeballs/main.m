...
*
*    Description:   NSTransport (NSTConnection) Objective-C API Example
*
*    @author: Joe Moulton
...
void LoadGameStateQuery(void)
{
    //Send the ReQL Query to find some URLs to download
    //Demonstrate issue of query and callback lambda using CXReQL API
    NSTRequestClosure queryCallback = ^void(CTError * __nullable error, NSTCursor* nstCursor)
    {
        fprintf(stderr, "LoadGameState response:  \n\n%.*s\n\n", (int)nstCursor.cursor->contentLength, nstCursor.cursor->file.buffer);
        CTCursorCloseMappingWithSize(nstCursor.cursor, nstCursor.cursor->contentLength);
        CTCursorCloseFile(nstCursor.cursor);
    };
    NSTReQL.db(@"GameState").table(@"Scene").get(@"BasicCharacterTest").run(_reqlCXConn, NULL, queryCallback);
}
...
void SendHTTPRequest(void)
{
    ...
    NSTURLRequest* getRequest = [NSTURL GET:urlPath responsePath:filepath];
    
    [getRequest setValue:@"3rdgen-sandbox-html-resources.s3.us-west-1.amazonaws.com:443" forHTTPHeaderField:@"Host"];
    ...
    NSTRequestClosure requestCallback = ^void(CTError * __nullable error, NSTCursor* nstCursor)
    {
    ...
    [getRequest sendOnConnection:_httpCXConn withCallback:requestCallback];
    
}

//int _nstURLConnectionClosure(CTError* err, NSTConnection* conn) {
...
//int _nsReQLConnectionClosure(CTError* err, NSTConnection* conn) {
NSTConnectionClosure _nstReQLConnectionClosure = ^int(CTError * err, NSTConnection * conn) {
...
void StartApplicationEventLoop(void)
{
    ...
    //TO DO:
#elif defined(__APPLE__)
    //create process wide event queue
    ...
    dispatch_queue_attr_t concurrentAttr = dispatch_queue_attr_make_with_qos_class(DISPATCH_QUEUE_CONCURRENT, QOS_CLASS_USER_INTERACTIVE, 0);
    dispatch_queue_t      processEventDispatchQueue = dispatch_queue_create(kCTProcessEventQueueID, concurrentAttr);
...
int main(int argc, const char * argv[])
{
    ...
    //On BSD & Darwin platforms we create thread/pool of threads to associate with each kqueue + pipe pair
    cxThread = CTThreadCreate(&cq, NST_Dequeue_Resolve_Connect_Handshake);
    rxThread = CTThreadCreate(&rq, NST_Dequeue_Recv_Decrypt);
    txThread = CTThreadCreate(&tq, NST_Dequeue_Encrypt_Send);
    ...
    //Use NSTransport NSTURL Obj-C API to connect to our HTTPS target server
    coroutine_bundle = NSTURL.connect(&httpTarget, _nstURLConnectionClosure);

    //Use NSTransport NSTReQL Obj-C API to connect to our RethinkDB service
    //coroutine_bundle = NSTReQL.connect(&reqlService, _nstReQLConnectionClosure);
