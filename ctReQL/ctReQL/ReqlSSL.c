...
BOOL ReqlCredentialHandleCreate( ReqlSSLContextRef * sslContextRef )
{
	SCHANNEL_CRED SchannelCred;
	SECURITY_STATUS  Status;
...
void ReqlSSLCertificateDestroy(ReqlSecCertificateRef * certRef)
{
	assert(*certRef);
#ifdef REQL_USE_MBED_TLS
	free(*certRef);
...
void ReqlSSLContextDestroy(ReqlSSLContextRef sslContextRef)
{
#ifdef REQL_USE_MBED_TLS	//DESTROY MBEDTLS CONTEXT
	//TO DO
 ...
 ***/
ReqlSecCertificateRef ReqlSSLCertificate( const char * caCertPath )
{
	ReqlSecCertificateRef caCert = NULL;
#ifdef REQL_USE_MBED_TLS
	int ret = 1;// , len;

	caCert = (ReqlSecCertificateRef)malloc(sizeof(ReqlSecCertificate));
 ...
 *	ReqlSSLDecrypt
 ***/
ReqlSSLStatus ReqlSSLDecryptMessage(ReqlSSLContextRef sslContextRef, void*msg, unsigned long *msgLength)
{
	int i;
	unsigned long length;
	ReqlSSLStatus scRet;				// unsigned long cbBuffer;    // Size of the buffer, in bytes
	...
	*/
	...
	if (pDataBuffer)
	{
		...
		if (length) // check if last two chars are CR LF
		{
			...
			*msgLength = length;
			...
			//if( buff[length-2] == 13 && buff[length-1] == 10 ) return ReqlSSLSuccess; // printf("Found CRLF\n");
	...
	if (pExtraBuffer)
	{
		...
		return ReqlSSLDecryptMessage(sslContextRef, msg, &(pExtraBuffer->cbBuffer));
	}
	//else
	//  cbIoBuffer = 0;
	return ReqlSSLSuccess;
...
ReqlSSLStatus ReqlSSLDecryptMessageInSitu(ReqlSSLContextRef sslContextRef, void**msg, unsigned long *msgLength)
{
	int i;
	unsigned long length;
	ReqlSSLStatus scRet;				// unsigned long cbBuffer;    // Size of the buffer, in bytes
 ...
 *
 ***/
ReqlSSLStatus ReqlSSLEncryptMessage(ReqlSSLContextRef sslContextRef, void*msg, unsigned long * msgLength)
{
	ReqlSSLStatus scRet;				// unsigned long cbBuffer;    // Size of the buffer, in bytes
 ...
 *
 ***/
ReqlSSLContextRef ReqlSSLContextCreate(REQL_SOCKET *socketfd, ReqlSecCertificateRef * certRef, const char * serverName)
{
	//OSStatus status;
	ReqlSSLContextRef sslContextRef = NULL;// = {0};
#ifdef REQL_USE_MBED_TLS
	int ret;
	...
	sslContextRef = (ReqlSSLContextRef)malloc(sizeof(ReqlSSLContext));
	memset(sslContextRef, 0, sizeof(ReqlSSLContext));
	...
	*/
	...
	if (!(sslContextRef = (ReqlSSLContextRef)malloc(sizeof(ReqlSSLContext))))
	{
	...
	if (!ReqlCredentialHandleCreate(&sslContextRef))
	{
		free(sslContextRef);
 ...
 ***/
int ReqlSSLHandshake(REQL_SOCKET socketfd, ReqlSSLContextRef sslContextRef, ReqlSecCertificateRef rootCertRef)
{
	int ret = 0;
 ...
 *
 ***/
int ReqlVerifyServerCertificate(ReqlSSLContextRef sslContextRef, char * pszServerName)
{
#ifdef REQL_USE_MBED_TLS
	uint32_t flags;
	//In real life, we probably want to bail out when ret != 0
 ...
 *	ReqlSSLRead
 ***/
size_t ReqlSSLRead( REQL_SOCKET socketfd, ReqlSSLContextRef sslContextRef, void * msg, unsigned long * msgLength )
...

{
  ...
  unsigned long              cbIoBuffer, cbData, cbIoBufferLength;
    ...
    {
        if( cbIoBuffer == 0 || scRet == ReqlSSLIncompleteMessage ) // get the data
        {
		...
		scRet = ReqlSSLDecryptMessage(sslContextRef, msg, &cbIoBuffer);
        ...
        if(scRet == ReqlSSLRenegotiate)
        {
			...
			scRet = ReqlSSLHandshake(socketfd, sslContextRef, NULL);
 ...
 *	ReqlSSLWrite
 ***/
int ReqlSSLWrite( REQL_SOCKET socketfd, ReqlSSLContextRef sslContextRef, void * msg, unsigned long * msgLength )// * phContext, PBYTE pbIoBuffer, SecPkgContext_StreamSizes Sizes )
...
// The encrypted message is encrypted in place, overwriting the original contents of its buffer.
{
	...
	errOrBytesWritten = ReqlSSLEncryptMessage(sslContextRef, msg, msgLength);
